...
export async function getLendMarkets(account: string): Promise<any[]> {
    ...
    for(let i = 0; i < +marketsLength.toString(); i++) {
        ...
        _markets.push({
            ...
            lendBalanceInterest: fromDecimals((BigInt(lendBalanceWithInterest.toString()) - BigInt(lendBalance.toString())).toString()),
            ...market
        })
    }
...
export async function getBorrowMarkets(account: string): Promise<any[]> {
    ...
    for(let i = 0; i < +marketsLength.toString(); i++) {
        ...
        temp.push({
            ...
            borrowBalanceInterest: fromDecimals((BigInt(borrowBalanceWithInterest.toString()) - BigInt(borrowBalance.toString())).toString()),
