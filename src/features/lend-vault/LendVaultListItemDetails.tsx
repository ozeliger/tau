...
const LendVaultListItem = ({ farm }) => {
    ...
    const borrowBalance = CurrencyAmount.fromRawAmount(tokenAddressToToken(farm.asset), farm.borrowBalanceWithDecimals ?? 0)
                ...
                <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
                    <div className="grid grid-cols-2 gap-4 p-4">
                        <div className="col-span-2 text-center md:col-span-1">
                            {farm.depositFeeBP && (
                                <div className="pr-4 mb-2 text-left cursor-pointer text-red">{`${`Deposit Fee`}: ${formatPercent(farm.depositFeeBP / 100)}`}</div>
                            )}
                            {account && (
                                <div className="pr-4 mb-2 text-left cursor-pointer text-secondary">
                                    {`Available`}: {" "}
                                    {formatNumberScale(available?.toFixed(2) ?? 0, false, 4)}
                                </div>
                            )}
                            ...
                            {approvalState === ApprovalState.NOT_APPROVED || approvalState === ApprovalState.PENDING ? (
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        ...
                                        addTransaction({ hash: tx.transactionHash, ...tx }, {
                                            summary: `${`Deposit`} ${
                                                farm.pair.token1
                        ...
                        <div className="col-span-2 text-center md:col-span-1">
                            ...
                            {account && (
                                <div className="pr-4 mb-2 text-left cursor-pointer text-secondary">
                                    {`Total Withdrawable`}: {" "}
                                    {formatNumberScale(lendBalance?.toFixed(2)) ?? 0}
                                </div>
                            )}
