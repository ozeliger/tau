...
export function useSolarMovrContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && SOLAR_FIL_PAIR[chainId], IUniswapV2PairABI, withSignerIfPossible)
}

export function useMovrUsdcContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && FIL_USDC_PAIR[chainId], IUniswapV2PairABI, withSignerIfPossible)
}

export function useRibMovrContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && RIB_FIL_PAIR[chainId], IUniswapV2PairABI, withSignerIfPossible)
}
