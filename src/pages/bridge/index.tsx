...
export default function Bridge() {
    ...
    const availableChains = [ChainId.HYPERSPACE, ChainId.MATIC_TESTNET]
    ...
    useEffect(() => {
        const meTokens = [
            chainFrom.id === ChainId.HYPERSPACE ? "wFIL" : "wMATIC",
            "DCT",
            "MAV",
            "WLR",
            "DIB"
        ...
        //                 return Tau.onChain(chainFromId)
        //             }
    ...
    const handleChainFrom = useCallback(
        (chain: Chain) => {
            ...
            if(changeTo.id !== ChainId.HYPERSPACE && chain.id !== ChainId.HYPERSPACE) {
    ...
    const handleChainTo = useCallback(
        (chain: Chain) => {
            ...
            if(changeFrom.id !== ChainId.HYPERSPACE && chain.id !== ChainId.HYPERSPACE) {
    ...
    const bridgeToken = async () => {
        ...
        try {
            ...
            bridge(
                ...
                addTransaction({ hash: receipt.transactionHash }, {
        ...
        <>
            <Modal isOpen={showConfirmation} onDismiss={() => setShowConfirmation(false)}>
                <div className="space-y-4">
                    ...
                    <Typography variant="sm" className="font-medium">
                        {`You will receive ${formatNumber(getAmountToReceive())} ${currency0?.symbol} on ${chainTo?.name}`}
                    </Typography>

                    {approvalState === ApprovalState.NOT_APPROVED || approvalState === ApprovalState.PENDING ? (
                        <Button color="gradient" size="lg" disabled={approvalState === ApprovalState.PENDING} onClick={approve}>
                    ...
                    <div className="p-4 space-y-4 rounded bg-dark-900" style={{ zIndex: 1 }}>
                        ...
                        <div className="p-4 text-center">
                            <div className="justify-between space-x-3 items-center">
                                <Typography component="h3" variant="base">
                                    {`Bridge tokens to and from the Hyperspace Network`}
                                </Typography>
                            </div>
                        </div>
                        ...
                        {currency0 && (
                            <div className={"p-2 sm:p-5 rounded bg-dark-800"}>
                                {MIN_SWAP_FEE > 0 && (
                                    <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                        <div className="text-sm font-medium text-secondary">
                                            Minimum Bridge Fee: {formatNumber(MIN_SWAP_FEE)}{" "}
                                            {tokenToBridge?.symbol}
                                        </div>
                                    </div>
                                )}
                                {MAX_SWAP_FEE > 0 && (
                                    <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                        <div className="text-sm font-medium text-secondary">
                                            Maximum Bridge Fee: {formatNumber(MAX_SWAP_FEE)}{" "}
                                            {tokenToBridge?.symbol}
                                        </div>
                                    </div>
                                )}
                                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                    <div className="text-sm font-medium text-secondary">
                                        Minimum Bridge Amount: {MIN_SWAP}{" "}
                                        {tokenToBridge?.symbol}
                                    </div>
                                </div>
                                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                    <div className="text-sm font-medium text-secondary">
                                        Maximum Bridge Amount: {MAX_SWAP}{" "}
                                        {tokenToBridge?.symbol}
                                    </div>
                                </div>
                                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                ...
                                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                                    <div className="text-sm font-medium text-secondary">
                                        Amounts greater than {formatNumber(320)}{" "}
                                        {tokenToBridge?.symbol} could take up to 12 hours.
                                    </div>
                                </div>
                            </div>
                        )}
