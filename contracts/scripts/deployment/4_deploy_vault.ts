...
async function main() {
    ...
    tauVault = await TauVault.deploy(
        chainlink.address,
        (.5 * 10**18).toString() // 500000000000000000
    )
    deployments.vault["TauVault"] = tauVault.address
    saveDeployments(deployments)

    await wait()

    const RewardControl = await ethers.getContractFactory("RewardControl")
    const rewardControl = await RewardControl.deploy(tauVault.address)
    ...
    for(const token in tokens) {
        if(supportedAssets.includes(token)) {
            ...
            const ib = `ib${token}`
            await tauVault._supportMarket(tokens[token], rateModel.address, deployments.vault.tokens[ib])
            await wait()
        }
    }
}
