...
async function main() { 
    ...
    deployments.tokens["wFIL"] = wxdc.address
    saveDeployments(deployments)

    await wait()

    const dct = await MockERC20.deploy(
        "DCT",
        "DCT",
        initialSupply
    )
    ...
    const dib = await MockERC20.deploy(
        "DIB",
        "DIB",
        initialSupply
    )
    ...
    // Wallaroo
    const wlr = await MockERC20.deploy(
        "WLR",
        "WLR",
        initialSupply
    )
    ...
    const mav = await MockERC20.deploy(
        "MAV",
        "MAV",
        initialSupply
    )
    ...
    const tau = await MockERC20.deploy(
        "TAU",
        "TAU",
        initialSupply
    )
    deployments.tokens["TAU"] = tau.address
    saveDeployments(deployments)
}
