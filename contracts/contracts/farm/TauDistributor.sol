...
import "./ITauERC20.sol";


contract TauDistributor is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _poolId, uint256 _amount) public nonReentrant {
        // require(
        //     block.number >= startBlock,
        //     "TauDistributor: Can not deposit before start"
        // );
    ...
    function safeTauTransfer(address _to, uint256 _amount) internal {
        if(tau.balanceOf(address(this)) > totalTauInPools) {
            //TauBal = total Tau in TauDistributor - total Tau in Tau pools, this will make sure that TauDistributor never transfer rewards from deposited Tau pools
