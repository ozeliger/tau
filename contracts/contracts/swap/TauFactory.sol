...
contract TauFactory is ITauFactory {
    bytes public constant creationCode = type(TauPair).creationCode; 
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(TauPair).creationCode));
        ...
        returns(address pair)
    {
        ...
        TauPair(pair).initialize(token0, token1);
